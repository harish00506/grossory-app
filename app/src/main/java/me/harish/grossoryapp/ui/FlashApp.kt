...
fun FlashApp(
   ...
   auth.currentUser?.let { flashViewModel.setUser(it) }
    val isVisible by flashViewModel.isVisiable.collectAsState()
                        ...
                        Row(
                            ...
                            Row(
                                ...
                                if (currentScreen == FlashAppScreen.Cart) {
                                    val sizeText = if (cartItems.size > 0) {
                                        "(${cartItems.size})"
                                    } else {
                                        ""
                                    }
                            ...
                            Row(
                                ...
                                Icon(imageVector = Icons.AutoMirrored.Filled.ArrowForward, contentDescription = "Log Out")
                                Text(
                                    text = "Logout",
                                    fontSize = 10.sp,
            ...
            NavHost(
                ...
                composable(route = FlashAppScreen.Start.name) {
                    StartScreen(
                        ...
                        onCategoryClicked = {
                            flashViewModel.updateSelectCategory(it)
...
fun AlertCheck(
    ...
    AlertDialog(
        onDismissRequest = { onNoButtonPressed() }, // Needed for outside-tap dismiss
        title = {
